...
- Complete refactor of validation logic to focus on Metalsmith-specific patterns
- Removed inappropriate server-oriented validations (caching, file I/O, DoS protection)
- Updated default validation checks to include `metalsmith-patterns`

**New Features:**

- Added `metalsmith-patterns` validation check for plugin factory patterns, function signatures, and metadata handling
- Refactored performance validations for in-memory object transformations (files object iteration, Buffer handling)
- Refactored security validations for build-time context (dependency security, error handling)
- Added comprehensive Metalsmith execution model documentation (METALSMITH_CONTEXT.md)

**Improvements:**

- Updated MCP schema to include all new validation check types
- Enhanced validation descriptions to reflect build-time focus
- Improved plugin pattern detection and recommendations
