...
const config = {
  ...
  describe( 'copyTemplateDirectory', function () {
    it( 'should copy multiple template files from a directory', async function () {
      ...
      await fs.writeFile( path.join( sourceDir, 'file1.js.template' ), 'export const name = "{{ name }}";' );
      await fs.writeFile( path.join( sourceDir, 'file2.md.template' ), '# {{ title }}' );
      await fs.writeFile( path.join( sourceDir, 'non-template.js' ), 'should not be copied' );
    ...
    it( 'should handle recursive directory copying', async function () {
      ...
      await fs.writeFile( path.join( sourceDir, 'root.txt.template' ), 'root: {{value}}' );
      await fs.writeFile( path.join( subDir, 'nested.txt.template' ), 'nested: {{value}}' );
