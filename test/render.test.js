...
function feature() {
  return true;
    ...
    it('should handle truthy values', function () {
      ...
      assert.equal(render(template, { value: 'string' }), 'Has value');
      assert.equal(render(template, { value: 1 }), 'Has value');
      // Nunjucks considers empty arrays as truthy (different from Handlebars)
      assert.equal(render(template, { value: [] }), 'Has value');
      assert.equal(render(template, { value: {} }), 'Has value');
    });
    ...
    it('should handle falsy values', function () {
      ...
      assert.equal(render(template, { value: '' }), '');
      assert.equal(render(template, { value: 0 }), '');
      assert.equal(render(template, { value: null }), '');
    });
  });
